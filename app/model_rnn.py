...
class RNNModel:
    def __init__(self, input_dim=5, hidden_dims=[128, 64], output_dim=1, dropout=0.2):
    ...
    def build_model(self):
        ...
        model.add(layers.Input(shape=(3, self.input_dim)))  

        # First RNN layer with return_sequences=True
        model.add(layers.SimpleRNN(self.hidden_dims[0], return_sequences=False, activation='relu'))
        model.add(layers.BatchNormalization())
        model.add(layers.Dropout(0.3))  

        # Second RNN layer
        model.add(layers.Dense(self.hidden_dims[1], activation = 'relu'))
        model.add(layers.BatchNormalization())
        model.add(layers.Dropout(0.3))  
